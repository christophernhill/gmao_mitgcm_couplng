GEOS routes the runoff to the ocean based on GEOS ocean mask. The runoff rerouting script output a binary file that contains information to GEOS where to reroute the runoff whenever the runoff is directed to a land point in MITgcm. The current implemetation is valid only for llc grids with cubed faces (nx=ny).
