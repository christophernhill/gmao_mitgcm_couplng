...
setenv SITE             NAS
setenv GEOSDIR          /nobackup/dmenemen/geos5/GEOSodas 
setenv GEOSBIN          /nobackup/dmenemen/geos5/GEOSodas/Linux/bin 
...
setenv    EXPID   TEST
setenv    EXPDIR  /nobackup/dmenemen/geos5/TEST
setenv    HOMDIR  /nobackup/dmenemen/geos5/TEST
...
setenv SSTDIR    /nobackupp8/dmenemen/GEOS5/experiments/llc90/rst
...
/bin/ln -sf $GRIDDIR/mit-llc90.ascii .
...
set H_NPX = `echo 180 | awk '{printf "%5.5i", $1}'`
set H_NPY = `echo 91 | awk '{printf "%5.5i", $1}'`
...
/bin/cp mitocean_run/STDOUT.0000* $EXPDIR/mit_output/STDOUT.${znIter00}
...
#DM         qsub $EXPDIR/forecasts/gcm_forecast.j${nymdz}-${nymdz}
     endif
endif
...
# for some reason the *.tar files are deleted somewhere - did not found 
# where so I archive them.
if ( ! -e archive_restart ) mkdir $EXPDIR/archive_restart
cp $EXPDIR/restarts/restarts.${edate}.tar $EXPDIR/archive_restart/

/bin/cp -rf RESTART $EXPDIR

if ( $rc == 0 ) then
      cd   $HOMDIR
#DM      if ( $capdate < $enddate ) qsub $HOMDIR/gcm_run.j
endif
