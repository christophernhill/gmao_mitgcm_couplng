...
# -------------------------------------------
  ifeq ($(ESMA_FC), pgfortran)
      ...
      ifeq ($(findstring openMpi,$(INC_MPI)),openMpi)
         ...
         LIB_MPI := -L$(subst include,lib,$(INC_MPI)) $(OPENMPI_LINK_FLAGS) -lmpi_cxx
      else
      ifeq ($(findstring openmpi,$(INC_MPI)),openmpi)
         OPENMPI_LINK_FLAGS := $(shell mpif90 -showme:link)
         LIB_MPI := -L$(subst include,lib,$(INC_MPI)) $(OPENMPI_LINK_FLAGS) -lmpi_cxx
      ...
      ifeq ($(PGI_MAJOR),16)
         #LIB_SYS = -ldl -lrt -lC $(GPU_TARGET)
         LIB_SYS = -ldl -lrt $(GPU_TARGET)
      endif
...
#   Darwin MPI default: assumes openmpi for now
#   -------------------------------------------
    FC := mpif90
...
#/opt/local/lib/gcc5/libstdc++.a
    LIB_SYS += -L$(GCC_DIR) -L$(GXX_DIR) -lstdc++ -lgcc_eh
# Line below is probably correct for clang 
...
# --------------------
  ifeq ($(ESMA_FC), gfortran) 
      ...
      CFLAGS   += $(FPIC) -Wno-missing-include-dirs
      fFLAGS   += $(FPIC) $(D)__GFORTRAN__ $(EXTENDED_SOURCE) $(NO_RANGE_CHECK) $(FPE) $(ALIGNCOM) -Wno-missing-include-dirs
      FFLAGS   += $(FPIC) $(D)__GFORTRAN__ $(EXTENDED_SOURCE) $(NO_RANGE_CHECK) $(FPE) $(ALIGNCOM) -Wno-missing-include-dirs
      f90FLAGS += $(FPIC) $(D)__GFORTRAN__ -ffree-line-length-none $(NO_RANGE_CHECK) $(FPE) $(ALIGNCOM) -Wno-missing-include-dirs
      F90FLAGS += $(FPIC) $(D)__GFORTRAN__ -ffree-line-length-none $(NO_RANGE_CHECK) $(FPE) $(ALIGNCOM) -Wno-missing-include-dirs
