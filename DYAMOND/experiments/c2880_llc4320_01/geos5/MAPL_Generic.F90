!  $Id: MAPL_Generic.F90,v 1.124 2017/08/09 20:22:08 bmauer Exp $
        ...
        if ( ESMF_AlarmIsRinging(STATE%RECORD%ALARM(I), RC=STATUS) ) then
           ...
           if (.not. ftype(filetype)) then
              ...
              if (K > 0) then
                 ...
                 filetypechar = lowercase(filetypechar)
...
! ErrLog Variables
  ...
  subroutine MAPL_GridCreate(GC, MAPLOBJ, ESMFGRID, srcGC, rc)
    ...
    character(len=9)                      :: imsz
    character(len=9)                      :: jmsz
    ...
    ! this block is for EASE grid only
    if( index(Gridname,'EASE') /=0) then
       pole='PE'
       date='DE'
       minIndex=(/0,0/)
       isEASE = .true.
       if(index(Gridname,'v2') /=0) then ! version 2 of EASE grid
          if(index(Gridname,'M36') /=0) then
             imsz='964'
             jmsz='406'
          endif
          if(index(Gridname,'M09') /=0) then
             imsz='3856'
             jmsz='1624'
          endif
          if(index(Gridname,'M03') /=0) then
             imsz='11568'
             jmsz='4872'
          endif
          if(index(Gridname,'M01') /=0) then
             imsz='34704'
             jmsz='14616'
          endif
       else ! version 1 EASE grid
          if(index(Gridname,'M36') /=0) then
             imsz='963'
             jmsz='408'
          endif
          if(index(Gridname,'M25') /=0) then
             imsz='1383'
             jmsz='586'
          endif
          if(index(Gridname,'M09') /=0) then
   ...
   if (date=='CF' .or. date=='DP') then
    ...
    if(STATUS/=ESMF_SUCCESS) then
       ! read IMS from a file to avoid line length limitation
       call MAPL_ReadImsJmsFromFile(STATE,'IMS',ims,NX,rc=status)
       if(STATUS/=ESMF_SUCCESS)  call MAPL_DecomposeDim ( IM_WORLD,ims,nx )
    end if

    call MAPL_GetResource( STATE, jms, 'JMS:', rc = status )
    if(STATUS/=ESMF_SUCCESS) then
       ! read JMS from a file to avoid line length limitation
       call MAPL_ReadImsJmsFromFile(STATE,'JMS',jms,NY,rc=status)
       if (STATUS/=ESMF_SUCCESS) call MAPL_DecomposeDim ( JM_WORLD,jms,ny )
    end if
...
! The coordinates of EASE grid will be calculated in GEOSldas application
    if (.not. isEASE ) then 
...
! Retrieve the coordinates so we can set them
    ...
    if(.not. EXACT) then
       ...
       if(STATUS/=ESMF_SUCCESS) then
       ! get exchange
       !DM   EXCH = STATE%EXCHANGEGRID
       !DM   ASSERT_(MAPL_LocStreamIsAssociated(EXCH))

       !DM   call MAPL_GridCoordAdjust(GRID, EXCH, RC=STATUS)
       !DM   VERIFY_(STATUS)
       else
...
!        VERIFY_(STATUS)
   ...
   subroutine MAPL_ReadImsJmsFromFile(state,lable,IJMS,N,RC)
      type (MAPL_MetaComp) , intent(INOUT)   :: STATE
      character(len=3) ,intent(in) :: lable
      integer ,intent(in) :: N
      integer,intent(inout) :: IJMS(:)
      ...
      Iam = "MAPL_ReadImsJmsFromFile"
      call ESMF_VmGetCurrent(VM, rc=status)
      VERIFY_(STATUS)

      if ( lable == "IMS") then
          call MAPL_GetResource( STATE, FNAME, 'IMS_FILE:', &
              default='IMS.rc', rc = status )
          VERIFY_(STATUS)
      else if (lable == "JMS") then
          call MAPL_GetResource( STATE, FNAME, 'JMS_FILE:', &
             default='JMS.rc', rc = status )
          VERIFY_(STATUS)
      else
          RETURN_(ESMF_FAILURE)
      endif
     ...
     call MAPL_CommsBcast(VM, IJMS, n=N, ROOT=MAPL_Root, rc=status)
     VERIFY_(STATUS)
     RETURN_(ESMF_SUCCESS)
